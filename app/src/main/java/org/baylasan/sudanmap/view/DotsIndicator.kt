package org.baylasan.sudanmap.view
...
class DotsIndicator : LinearLayout {
    ...
    private val mInternalPageChangeListener = object : ViewPager2.OnPageChangeCallback() {
        ...
        override fun onPageSelected(position: Int) {
            ...
            if (mLastPosition >= 0) getChildAt(mLastPosition)?.setBackgroundResource(
    ...
    private fun checkIndicatorConfig() {
        mIndicatorWidth =
            if (mIndicatorWidth < 0) dpTopx(DEFAULT_INDICATOR_WIDTH.toFloat()) else mIndicatorWidth
        mIndicatorHeight =
            if (mIndicatorHeight < 0) dpTopx(DEFAULT_INDICATOR_WIDTH.toFloat()) else mIndicatorHeight
        mIndicatorMargin =
            if (mIndicatorMargin < 0) dpTopx(DEFAULT_INDICATOR_WIDTH.toFloat()) else mIndicatorMargin
        mIndicatorBackgroundResId =
            if (mIndicatorBackgroundResId == 0) R.drawable.ic_dot_lightgrey else mIndicatorBackgroundResId
        mIndicatorUnselectedBackgroundResId =
